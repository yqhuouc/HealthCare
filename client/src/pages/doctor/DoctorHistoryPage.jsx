...
function DoctorHistoryPage() {
  ...
  const filtered = appointments.filter((record) => {
    ...
    const aptDate = new Date(record.ngayDat).toLocaleDateString("en-CA", { timeZone: "Asia/Ho_Chi_Minh" });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div>
          <h1 className="text-2xl font-black text-slate-800">Lịch sử khám bệnh</h1>
          <p className="text-slate-500 text-sm font-medium">Hệ thống lưu trữ hồ sơ bệnh án đã hoàn thành</p>
        </div>
      </div>
      ...
      <div className="bg-white p-4 sm:p-5 rounded-xl border border-slate-100 shadow-sm transition-all hover:shadow-md">
        <div className="grid md:grid-cols-12 gap-4">
          ...
          <div className="md:col-span-3">
            ...
            <div className="relative">
              <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-xl font-light">calendar_today</span>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      {record.benhNhan?.taiKhoan?.anhDaiDien ? (
                        <img src={getPatientAvatar(record.benhNhan.taiKhoan.anhDaiDien)} alt={record.benhNhan?.hoTen} className="size-10 rounded-full object-cover shrink-0 border-2 border-white shadow-sm" onError={(e) => { e.target.onerror = null; e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(record.benhNhan?.hoTen || "BN")}&size=128&background=f1f5f9&color=64748b`; }} />
                      ) : (
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-bold text-slate-800 truncate leading-tight">{record.benhNhan?.hoTen || "—"}</p>
                        <p className="text-[10px] font-black text-primary uppercase tracking-widest">{code}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-[11px] font-black text-slate-400">{formatDate(record.ngayDat)}</p>
                    </div>
                  </div>
                  <p className="text-xs text-slate-500 line-clamp-2 bg-slate-50 p-2 rounded">
                    <span className="font-bold text-slate-700">Chẩn đoán:</span> {record.donThuoc?.chanDoan || record.lyDoKham || "—"}
                  </p>
                  <Link
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50/80 border-b border-slate-100">
                <th className="text-left px-6 py-4 text-[11px] font-black text-slate-400 uppercase tracking-widest">Ngày khám</th>
                <th className="text-left px-6 py-4 text-[11px] font-black text-slate-400 uppercase tracking-widest">Mã Bệnh Nhân</th>
                <th className="text-left px-6 py-4 text-[11px] font-black text-slate-400 uppercase tracking-widest">Họ tên Bệnh Nhân</th>
                <th className="text-left px-6 py-4 text-[11px] font-black text-slate-400 uppercase tracking-widest">Chẩn đoán / Lý do</th>
                <th className="text-center px-6 py-4 text-[11px] font-black text-slate-400 uppercase tracking-widest">Thao tác</th>
              </tr>
            </thead>
                ...
                return (
                  <tr key={record.id} className="hover:bg-slate-50/30 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        {record.benhNhan?.taiKhoan?.anhDaiDien ? (
                          <img src={getPatientAvatar(record.benhNhan.taiKhoan.anhDaiDien)} alt={record.benhNhan?.hoTen} className="size-9 rounded-full object-cover shrink-0 border border-slate-100" onError={(e) => { e.target.onerror = null; e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(record.benhNhan?.hoTen || "BN")}&size=128&background=f1f5f9&color=64748b`; }} />
                        ) : (
                        ...
                        <span className="text-sm font-bold text-slate-800">{record.benhNhan?.hoTen || "—"}</span>
                      </div>
                    </td>
