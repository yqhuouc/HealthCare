...
function DoctorAppointmentsPage() {
  ...
  const effectiveDate = activeDateTab === "today" ? todayStr : activeDateTab === "tomorrow" ? tomorrowStr : customDate;
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium">
            Bác sĩ hãy chọn ngày và bệnh nhân để bắt đầu quy trình khám bệnh
          </p>
        </div>
      </div>
        ...
        <div className="md:hidden">
          {filteredAppointments.length > 0 ? (
                ...
                return (
                  <div key={apt.id} className="p-4 space-y-4">
                    <div className="flex items-start justify-between">
                      ...
                      <span
                        className={`px-2 py-0.5 rounded border text-[10px] font-bold uppercase leading-none ${badge.className}`}
                      >
                        {badge.label}
                      </span>
                    </div>
                    ...
                    <div className="flex gap-2">
                      ...
                      {apt.trangThai === 0 && (
                        <button
                          ...
                          className="flex-1 py-2 bg-blue-600 text-white rounded-lg text-[10px] font-bold uppercase tracking-wider"
                        >
                          Xác nhận
                        </button>
                      )}
                      {apt.trangThai === 1 && (
                        <Link
                          ...
                          className="flex-1 py-2 bg-emerald-600 text-white text-center rounded-lg text-[10px] font-bold uppercase tracking-wider"
                        >
                          Bắt đầu khám
                        </Link>
                      )}
                      {apt.trangThai === 2 && (
                        <Link
                          to={`/doctor/appointments/${apt.id}`}
                          className="flex-1 py-2 border-2 border-primary text-primary text-center rounded-lg text-[10px] font-bold uppercase tracking-wider"
                        >
                          Xem hồ sơ
                        </Link>
                      )}
                      ...
                      <button
                        ...
                        className="px-4 py-2 border-2 border-slate-200 text-slate-400 rounded-lg text-[10px] font-bold uppercase"
                      >
                        Hủy
                      </button>
                    </div>
            ...
            <div className="py-20 text-center text-slate-400 font-bold italic text-sm">
              Trống: Không có lịch khớp với bộ lọc
            </div>
          )}
        </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-200">
                ...
                <th className="px-6 py-4 text-[10px] font-bold uppercase text-slate-400 tracking-widest text-center">
                  Trạng thái
                </th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredAppointments.length > 0 ? (
                    ...
                    <tr key={apt.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-3">
                          <img
                            src={
                              getPatientAvatar(apt.benhNhan?.taiKhoan?.anhDaiDien) ||
                              `https://ui-avatars.com/api/?name=${encodeURIComponent(apt.benhNhan?.hoTen || "BN")}&size=128&background=f1f5f9&color=64748b`
                            }
                            alt="Avatar"
                      ...
                      <td className="px-6 py-5 text-center">
                        <div className="inline-flex flex-col gap-1 items-center">
                          <span
                            className={`px-2 py-0.5 rounded border text-[10px] font-bold uppercase ${badge.className}`}
                          >
                            {badge.label}
                          </span>
                          {apt.donThuoc && (
                            <span className="text-[8px] font-bold text-emerald-600 bg-emerald-50 px-1 border border-emerald-100 rounded">
                              ĐÃ KÊ ĐƠN
                            </span>
                          )}
                        </div>
                ...
                <tr>
                  <td colSpan={6} className="py-24 text-center text-slate-400 font-bold italic">
                    Không có dữ liệu lịch khám phù hợp.
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 pb-10">
        ...
        <div className="p-5 rounded-xl border-2 border-emerald-100 bg-emerald-50/20">
          <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest leading-none mb-2">
            Hoàn thành
          </p>
