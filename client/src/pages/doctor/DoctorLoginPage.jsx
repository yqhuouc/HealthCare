...
const MEDICAL_ICONS = ["medical_services", "stethoscope", "local_hospital", "vaccines", "healing", "monitor_heart", "pills", "psychiatry"];
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (user.vaiTro !== "bac_si" && user.vaiTro !== "admin") {
        toast.error("Truy cập bị từ chối! Tài khoản này không có quyền bác sĩ.");
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center p-8 lg:p-20 bg-white shadow-2xl z-10">
        <div className="max-w-md mx-auto w-full space-y-10">
          ...
          <div className="space-y-4">
            <Link to="/" className="inline-flex items-center gap-2.5 text-primary group transition-all">
              <span className="material-symbols-outlined text-4xl font-black bg-primary/10 p-2 rounded-2xl group-hover:scale-110 transition-transform">medical_services</span>
              <span className="text-2xl font-black tracking-tighter">HealthCare<span className="text-slate-300">.</span></span>
            </Link>
            <div>
              <h2 className="text-4xl font-black text-slate-800 tracking-tight">Cổng Nội bộ</h2>
              <p className="text-slate-400 font-bold text-xs uppercase tracking-widest mt-2 italic">Dành riêng cho Bác sĩ & Quản trị viên</p>
            </div>
          </div>
          ...
          <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="space-y-1">
              ...
              <div className="relative group">
                ...
                />
                <span className="absolute right-4 top-1/2 -translate-y-1/2 material-symbols-outlined text-slate-300 group-focus-within:text-primary transition-colors">mail</span>
              </div>
              {errors.email && (
                <p className="text-red-500 text-[10px] font-black uppercase mt-1.5 ml-1">{errors.email.message}</p>
              )}
            </div>
            ...
            <div className="space-y-1">
              ...
              <div className="relative group">
                <input
                  ...
                  {...register("password", {
                    ...
                    minLength: { value: 6, message: "Mật khẩu an toàn phải từ 6 ký tự" },
                  })}
                />
                <button
                  type="button"
              ...
              {errors.password && (
                <p className="text-red-500 text-[10px] font-black uppercase mt-1.5 ml-1">{errors.password.message}</p>
              )}
            </div>
    ...
    <div className="hidden lg:flex lg:w-1/2 relative flex-col justify-center p-16 text-white overflow-hidden bg-linear-to-br from-primary via-primary/95 to-slate-900">
      ...
      <div className="relative z-10 space-y-8">
        <div className="flex items-center gap-4">
          <span className="material-symbols-outlined text-6xl font-black bg-white/10 backdrop-blur-md p-4 rounded-3xl border border-white/20">clinical_notes</span>
        </div>
        <div className="space-y-4">
          ...
          <p className="text-xl text-white/70 font-medium leading-relaxed max-w-sm">
            Công cụ hỗ trợ bác sĩ tối ưu hóa quy trình làm việc, tập trung hoàn toàn vào việc chăm sóc sức khỏe cộng đồng.
          </p>
        </div>
        ...
        <div className="flex gap-10 pt-8 border-t border-white/10">
          <div>
            ...
            <p className="text-[10px] font-black uppercase text-white/40 tracking-widest">Ca khám/tháng</p>
          </div>
          <div>
            <p className="text-3xl font-black">99%</p>
            <p className="text-[10px] font-black uppercase text-white/40 tracking-widest">Hài lòng</p>
          </div>
        </div>
      </div>
