...
import { useSlotTrong, useCreateAppointment, useAppointmentsByPatient } from "../../hooks/queries/useAppointmentQueries";
...
export default function BookingPage() {
  ...
  // TanStack Query: Lấy danh sách lịch hẹn của bệnh nhân để kiểm tra chặn đặt nhiều lịch
  const { data: patientAptsRes, isLoading: loadingPatientApts } = useAppointmentsByPatient(benhNhanId);
  const patientApts = patientAptsRes?.data || [];
  ...
  // Kiểm tra lịch hẹn chưa hoàn tất (0: Chờ xác nhận, 1: Đã xác nhận)
  const hasActiveAppointment = useMemo(() => {
    if (user?.vaiTro !== "benh_nhan") return false;
    return patientApts.some((apt) => apt.trangThai === 0 || apt.trangThai === 1);
  }, [patientApts, user?.vaiTro]);

  // Loading thông tin bác sĩ hoặc lịch sử đặt lịch để kiểm tra chống spam
  const isDataLoading = loadingDoctor || (!!benhNhanId && loadingPatientApts);
  ...
  return (
    <div className="max-w-3xl mx-auto py-12 px-4">
      ...
      {hasActiveAppointment ? (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6 text-center max-w-lg mx-auto my-8 space-y-4 shadow-sm animate-fade-in">
          ...
          <h3 className="text-lg font-bold text-slate-800">Không thể đặt thêm lịch hẹn mới</h3>
          <p className="text-sm text-slate-600 leading-relaxed">
            Bạn đang có một lịch hẹn chưa hoàn tất (đang chờ xác nhận hoặc đã được xác nhận). 
            Để tránh tình trạng spam, hệ thống giới hạn mỗi bệnh nhân chỉ được phép đặt tối đa 1 lịch hẹn hoạt động cùng lúc.
          </p>
          <p className="text-xs text-slate-400">
            Vui lòng hoàn tất hoặc hủy lịch hẹn hiện tại trước khi đặt lịch mới.
          </p>
