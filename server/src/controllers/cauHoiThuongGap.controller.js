/**
 * Controller FAQ: trang public chỉ FAQ active; admin CRUD + danh sách đầy đủ.
 * Lỗi → asyncHandler → errorHandler.
 */
const { asyncHandler } = require("../middlewares/error.middleware");
const cauHoiThuongGapService = require("../services/cauHoiThuongGap.service");

// GET /api/cau-hoi-thuong-gap — FAQ đang hoạt động (public)
const getActive = asyncHandler(async (req, res) => {
  const faqs = await cauHoiThuongGapService.getActive();
  res.json({ success: true, data: faqs });
});

// GET /api/cau-hoi-thuong-gap/all — tất cả + phân trang (admin)
...
// GET /api/cau-hoi-thuong-gap/:id — chi tiết một FAQ
const getById = asyncHandler(async (req, res) => {
  const faq = await cauHoiThuongGapService.getById(req.params.id);
  res.json({ success: true, data: faq });
});

// POST /api/cau-hoi-thuong-gap — tạo (admin)
...
// PUT /api/cau-hoi-thuong-gap/:id — cập nhật (admin)
...
// DELETE /api/cau-hoi-thuong-gap/:id — xóa (admin)
